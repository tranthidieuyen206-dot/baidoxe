...
public class Database {
    ...
    public void insertData(String vitri, String bienso, String loai, String mau, String gio) {
        String sSQL = "INSERT INTO server5 VALUES ('" + vitri + "','" + bienso + "','" + loai + "','" + mau + "','" + gio + "')";
        try {
    ...
    public void delData(String id) {
        try {
            String sSQL = "DELETE FROM server5 WHERE vitri='" + id + "'";
            stmt.executeUpdate(sSQL);
        } catch (Exception e) {
            System.out.println("SQLException: " + e.getMessage());
        }
    ...
    public boolean isEmpty(String id) {
        boolean check = true;
        try {
            String sSQL = "SELECT vitri FROM server5 WHERE vitri='" + id + "'";
            rs = stmt.executeQuery(sSQL);
            if (rs.next()) {
                check = false;
            }
    ...
    public boolean querySQL(String vitri, String bienso, String hieu, String mau) {
        ...
        try {
            ...
            String sSQL = "SELECT * FROM server5 WHERE vitri='" + vitri + "'"
                    + "AND bienso='" + bienso + "'"
                    + "AND hieu='" + hieu + "'"
                    + "AND mau='" + mau + "'";
            rs = stmt.executeQuery(sSQL);
            //ResultSetMetaData rsmd = rs.getMet5Data();
